<template>
  <div>
      ...
      <a-table
        ...
        <template #bodyCell="{column, record}">
          ...
          <template v-if="column.dataIndex === 'action'">
            <a-button type="primary" @click="getDeploymentDetail(record)" ghost>
              <FormOutlined />
              <span style="margin-left: 3px">详情</span>
            </a-button>
            <a-button danger style="margin-left: 5px" @click="showConfirm('删除', record, deleteDeployment)" ghost>
              <DeleteOutlined />
            ...
            <a-button class="log-btn" style="margin-left: 5px; margin-top: 5px" type="primary" @click="showConfirm('重启', record, restartDeployment)" ghost>
              <ReloadOutlined />
    ...
    <a-modal
        v-model:visible="deploymentReplicasModal"
        title="副本数调整"
        :confirm-loading="appLoading"
        cancelText="取消"
        okText="更新"
        @ok="updateDeploymentReplicas">
      <div style="text-align:center">
        <span style="margin-right:30px;">实例数: </span>
        <a-input-number v-model:value="deploymentReplicasNum" :min="0" :max="30" label="描述文字"></a-input-number>
        <a-popover>
          <template #content>
    ...
    <a-drawer
        ...
        @close="onClose">
      <br>
      <a-form ref="formRef" :model="deploymentCreate" :labelCol="{style: {width: '30%'}}">
        <a-form-item
            label="名称"
            name="Name"
...
export default {
  ...
  setup() {
    ...
    const deploymentUpdate = reactive({
    ...
    const deploymentReplicasNum = ref(0)
    const deploymentReplicasModal = ref(false)
    const deploymentUpdateReplicas = reactive({
      url: common.k8sDeploymentReplicas,
      params: {
        namespace: '',
        cluster: '',
        deployment_name: '',
    ...
    const deploymentRestart = reactive({
    ...
    const deploymentDelete = reactive({
    ...
    const deploymentCreate = reactive({
      Name: '',
      Namespace: '',
      Replicas: 1,
      Image: '',
    ...
    const deploymentCreateData = reactive({
    ...
    function getDeploymentList() {
      ...
      httpClient.get(deploymentListData.url,{params: deploymentListData.params}).then(res => {
    ...
    function getDeploymentDetail(e) {
      ...
      httpClient.get(deploymentDetailData.url,{params: deploymentDetailData.params}).then(res => {
    ...
    function updateDeployment() {
      ...
      deploymentUpdate.params.namespace = namespaceValue.value
      deploymentUpdate.params.content = content
      deploymentUpdate.params.cluster = localStorage.getItem('k8s_cluster')
      httpClient.put(deploymentUpdate.url, deploymentUpdate.params).then(res => {
        message.success("更新Deployment成功")
      }).catch(err => {
    ...
    function handleReplicasModal(e) {
      deploymentReplicasModal.value = true
      deploymentDetail.metadata = e.metadata
      deploymentReplicasNum.value = e.spec.replicas
    }
    function updateDeploymentReplicas() {
      appLoading.value = true
      deploymentUpdateReplicas.params.deployment_name = deploymentDetail.metadata.name
      deploymentUpdateReplicas.params.namespace = deploymentDetail.metadata.namespace
      deploymentUpdateReplicas.params.replicas = deploymentReplicasNum.value
      deploymentUpdateReplicas.params.cluster = localStorage.getItem('k8s_cluster')
      httpClient.put(deploymentUpdateReplicas.url, deploymentUpdateReplicas.params).then(res => {
        message.success("更新Deployment副本数成功")
      }).catch(err => {
        message.error("更新Deployment副本数失败")
      }).finally(() => {
        getDeploymentList()
        deploymentReplicasModal.value = false
      })
    }
    function restartDeployment(e) {
      appLoading.value = true
      deploymentRestart.params.deployment_name = e.metadata.name
      deploymentRestart.params.namespace = e.metadata.namespace
      deploymentRestart.params.cluster = localStorage.getItem('k8s_cluster')
      httpClient.put(deploymentRestart.url, deploymentRestart.params).then(res => {
        message.success("重启Deployment成功")
      }).catch(err => {
    ...
    function deleteDeployment(e) {
      appLoading.value = true
      deploymentDelete.params.deployment_name = e.metadata.name
      deploymentDelete.params.namespace = e.metadata.namespace
      deploymentDelete.params.cluster = localStorage.getItem('k8s_cluster')
      httpClient.delete(deploymentDelete.url, {data: deploymentDelete.params}).then(res => {
    ...
    function createDeployment() {
      ...
      if (!regex.test(deploymentCreate.Labels)) {
      ...
      let label = new Map ()
      let a = (deploymentCreate.Labels).split(",")
      a.forEach(item => {
        let b = item.split("=")
        label[b[0]] = b[1]
      })
      let cpu, memory
      let resourceList = deploymentCreate.Resources.split('/')
      cpu = resourceList[0]
      memory = resourceList[1] + "Gi"
      deploymentCreateData.params.label = label
      deploymentCreateData.params.cpu = cpu
      deploymentCreateData.params.memory = memory
      deploymentCreateData.params.name = deploymentCreate.Name
      deploymentCreateData.params.namespace = deploymentCreate.Namespace
      deploymentCreateData.params.replicas = deploymentCreate.Replicas
      deploymentCreateData.params.image = deploymentCreate.Image
      deploymentCreateData.params.health_check = deploymentCreate.Health_Check
      deploymentCreateData.params.health_path = deploymentCreate.Health_Path
      deploymentCreateData.params.container_port = parseInt(deploymentCreate.Container_Port)
      deploymentCreateData.params.cluster = localStorage.getItem('k8s_cluster')
      httpClient.post(deploymentCreateData.url, deploymentCreateData.params).then(res => {
        message.success("创建Deployment成功")
      }).catch(err => {
        message.error("创建Deployment失败")
      }).finally(() => {
        resetForm()
        getDeploymentList()
    ...
    return {
      ...
      deploymentReplicasNum,
      createDrawer,
      formRef,
      ...toRefs(deploymentCreate),
      timeTrans,
      ellipsis,
      handleTableChange,
      getSearchValue,
